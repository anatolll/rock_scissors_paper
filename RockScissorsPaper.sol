...
function decodeHash1(uint id, string seed1) public {
    require(msg.sender == rooms[id].player1 && rooms[id].counter == 3 && keccak256(seed1) == rooms[id].hash1 && rooms[id].open == true);
...
function decodeHash2(uint id, string seed2) public {
    require(msg.sender == rooms[id].player2 && rooms[id].counter == 3 && keccak256(seed2) == rooms[id].hash2 && rooms[id].open == true);
...
function str_to_bytes_1(string seed) internal returns(bytes1){
    bytes memory seed_b;
    seed_b = bytes(seed);
    return seed_b[0];
}
